...
```bash
curl -sI https://dev.boutique.example.com || true
curl -sI https://stage.boutique.example.com || true
```
...
In the Argo CD UI (**https://argocd.boutique.example.com**) or via CLI:
...
```bash
BUCKET=YOUR_TFSTATE_BUCKET   # e.g. tfstate-YOUR_PROJECT_ID
